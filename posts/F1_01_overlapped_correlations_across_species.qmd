...
Create dataframe for c1 and c2 network.
...
rmsk <- hg19rmsk_info
rownames(rmsk) <- seq(1:nrow(rmsk))
rmsk[rmsk$class_id=="SVA", 2] <- "SVA"
...
hsc1.sig.select <- hsc1.sig.select %>%
    ...
    left_join(rmsk[,c(1,2)], join_by(teName==gene_id))
colnames(hsc1.sig.select)[7] <- "teFamily"

hsc2.sig.select <- hsc2.sig.select %>%
    mutate(age=ifelse(geneName %in% young_kznf$external_gene_name | 
                          teName %in% te_infer$NM, "young", "old")) %>%
    left_join(rmsk[,c(1,2)], join_by(teName==gene_id))
colnames(hsc2.sig.select)[7] <- "teFamily"
...
rownames(hsc1.sig.select) <- 1:nrow(hsc1.sig.select)

hsc2.sig.select <- hsc2.sig.select %>%
    mutate(znfAge = ifelse(geneName %in% young_kznf$external_gene_name, "young", "old")) %>%
    mutate(teAge = ifelse(teName %in% te_infer$NM, "young", "old"))
rownames(hsc2.sig.select) <- 1:nrow(hsc2.sig.select)
