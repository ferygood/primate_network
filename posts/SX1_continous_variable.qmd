...
g_hm_kznf <- ggplot(hm_kznf, aes(x=taxo, y=log(baseMean))) +
    geom_boxplot(fill = "#C087F8", color="black") +
    xlab("") +
    ylab("log(exp)") +
    theme_bw() +
...
g_hm_te <- ggplot(hm_TE, aes(x=species, y=log(baseMean))) +
    geom_boxplot(fill = "#76D7C4", color="black") +
    xlab("") +
    ylab("log(exp)") +
    theme_bw() +
...
ggsave("../figures/continuous_age_check_hmc1.jpg", dpi=400, width=6)
```
