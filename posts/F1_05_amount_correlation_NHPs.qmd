...
|-------------------|----------------|------------------|------------|
| chimpanzee        | 933/954/1887   | 92               | 364        |
| bonobo            | 1093/1438/2531 | 65               | 350        |
| macaque           | 408/570/978    | 75               | 351        |

| sample (cluster2) | -/+/all       | unique KRAB-ZNFs | unique TEs |
|-------------------|---------------|------------------|------------|
| chimpanzee        | 601/435/1036  | 74               | 293        |
| bonobo            | 302/1332/1634 | 63               | 285        |
| macaque           | 508/380/888   | 69               | 303        |
...
# cluster 1
pt_c1_filter <- pt_corr_c1 %>% filter(pair %in% hm_c1$x)
pp_c1_filter <- pp_corr_c1 %>% filter(pair %in% hm_c1$x)
mm_c1_filter <- mm_corr_c1 %>% filter(pair %in% hm_c1$x)
...
# cluster 2
pt_c2_filter <- pt_corr_c2 %>% filter(pair %in% hm_c2$x )
pp_c2_filter <- pp_corr_c2 %>% filter(pair %in% hm_c2$x)
mm_c2_filter <- mm_corr_c2 %>% filter(pair %in% hm_c2$x)
